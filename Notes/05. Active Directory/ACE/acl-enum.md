...
#### Get domain object ACLs for specific SIDs
```powershell
foreach ($key in $userSIDs.Keys) {
 Get-DomainObjectACL -ResolveGUIDs -Identity * | Where-Object {$_.SecurityIdentifier -eq $userSIDs[$key]}
}
```
...
Get-ADObject -SearchBase "CN=Extended-Rights,$((Get-ADRootDSE).ConfigurationNamingContext)" -Filter {ObjectClass -like 'ControlAccessRight'} -Properties * | Where-Object {$_.rightsGuid -eq $guid} | Select-Object Name, DisplayName, DistinguishedName, rightsGuid
```
...
#### Enumerate ACLs using Get-Acl
```powershell
$users = Get-Content "C:\Users\htb-student\Desktop\ad_users.txt"
foreach ($user in $users) {
 Get-Acl "AD:\$(Get-ADUser $user)" | Select-Object Path -ExpandProperty Access | Where-Object {$_.IdentityReference -match 'INLANEFREIGHT\\wley'}
}
```
